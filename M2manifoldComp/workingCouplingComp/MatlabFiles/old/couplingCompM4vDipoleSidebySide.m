%% Comparison of Manifolds of 4 element array, parallel side by side
    %Patrick Ellis
    %April 8th, 2015       
...
%Compute 4x4 Analytic Coupling Matrix
    ...
    Z12 = computeMIparallel(len,max( abs(p_all(1,:) - p_all(2,:)) ),beta);
        Z21 = Z12;
    Z13 = computeMIparallel(len,max( abs(p_all(1,:) - p_all(3,:)) ),beta);
        Z31 = Z13;
    Z14 = computeMIparallel(len,max( abs(p_all(1,:) - p_all(4,:)) ),beta);
        Z41 = Z14;
    Z23 = computeMIparallel(len,max( abs(p_all(2,:) - p_all(3,:)) ),beta);
        Z32 = Z23;
    Z24 = computeMIparallel(len,max( abs(p_all(2,:) - p_all(4,:)) ),beta);
        Z42 = Z24;
    Z34 = computeMIparallel(len,max( abs(p_all(3,:) - p_all(4,:)) ),beta);
        Z43 = Z34;   
