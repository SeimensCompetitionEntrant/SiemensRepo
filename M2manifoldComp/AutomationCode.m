...
tic;
for i = 1:numAnts
  antFileName = strcat('ant',num2str(i));
  ...
  fprintf(fileID, '%6s%d%10s\r', 'EX  0 ', i , ' 5 0 1 0 0');
...
if numAnts != 1
  system(strcat({'COPY NUL '},simulFileName,'.nec'));
  fileID = fopen(strcat(simulFileName,'.nec'),'wt');
  fprintf(fileID,'%s\r%s\r','CM','CE');
  for k = 1:numAnts
    fprintf(fileID, '%4s%d%6s%f%1s%f%3s%f%1s%f%6s\r', 'GW  ', k, '  9 0 ', (k-1)*dist, ' ', -wvl/4, ' 0 ', (k-1)*dist, ' ', wvl/4, ' .0001');
  end
...
if numAnts != 1
  movefile(strcat(simulFileName,'.nec'), strcat('.\workingCouplingComp\NECuserMadeFiles\vDipoleArray\test\', simulFileName, '.nec'));
  system(strcat({'4nec2.exe '}, folderPath, '\workingCouplingComp\NECuserMadeFiles\vDipoleArray\test\', simulFileName, '.nec -I'));
  if exist(strcat('.\workingCouplingComp\NECuserMadeFiles\vDipoleArray\test\', simulFileName, '.txt'),'file') != 0
    delete(strcat('.\workingCouplingComp\NECuserMadeFiles\vDipoleArray\test\', simulFileName, '.txt'));
  end
  movefile(strcat(necPath, '\out\', simulFileName, '.out'),strcat('.\workingCouplingComp\NECuserMadeFiles\vDipoleArray\test\', simulFileName, '.txt'));
end
timeElapsed = toc;
fprintf('%s%f','4nec2 Time: ',timeElapsed);
cd('.\workingCouplingComp\MatlabFiles');
M2vArrayFinal(numAnts, dist, folderPath, 5, 5);
cd('..\..');